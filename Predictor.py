...
for i in range(num_predictions): 
    ...
    filtered_data = data[(data['1st_number'].between(1, 45)) & 
                         (data['2nd_number'].between(1, 45)) & 
                         (data['3rd_number'].between(1, 45)) & 
                         (data['4th_number'].between(1, 45)) & 
                         (data['5th_number'].between(1, 45)) & 
                         (data['6th_number'].between(1, 45))]
    ...
    new_data = pd.DataFrame({
        "1st_number": [randint(1, 45) for _ in range(100)],
        "2nd_number": [randint(1, 45) for _ in range(100)],
        "3rd_number": [randint(1, 45) for _ in range(100)],
        "4th_number": [randint(1, 45) for _ in range(100)],
        "5th_number": [randint(1, 45) for _ in range(100)],
        "6th_number": [randint(1, 45) for _ in range(100)],
    })

    # Sử dụng mô hình đã được huấn luyện để dự đoán 6 số tiếp theo cho mỗi bộ đặc trưng
    predictions = model.predict(new_data)

    # Lấy bộ số có khả năng cao nhất dựa trên các dự đoán
